 ...
 * Uninstall cleanup.
 *
 * Freemius manages the plugin uninstall lifecycle, so we cannot ship a
 * standalone uninstall.php (WordPress would run it instead of letting Freemius
 * clean up its own licensing/account data). Instead, this logic is registered
 * on Freemius's `after_uninstall` hook in the main plugin file.
 *
...
{
    /**
     * Purge all plugin data: custom tables, options, and meta.
     *
     * Fired by Freemius via `asc_fs()->add_action('after_uninstall', ...)`.
     *
     * @return void
     */
