...
class Sitemap
{
    private const QUERY_VAR = 'ai_seo_captain_sitemap';
    ...
    private function build_sitemap(string $type, array $options): string
    {
        ...
        if ('post' === $type && ! empty($options['sitemap_include_posts'])) {
            ...
            $urls = apply_filters('ai_seo_captain_sitemap_urls', array(), $type);
        }
    ...
    {
        $noindex_meta_key = '_ai_seo_captain_robots_directives';
        ...
        foreach ($posts as $post) {
            ...
            $is_cornerstone = ! empty(get_post_meta($post->ID, '_ai_seo_captain_cornerstone', true));
...
<body>
    <h1>XML Sitemap</h1>
    <p class="desc">Generated by SEO Captain for ' . $site_name . '</p>
    <xsl:choose>
